...
def item_from_dataset(
    ...
    if isinstance(dataset, FnetDataset):
        ...
        item = dataset[idx]
    target = None
    # Accept tuple or list from datasets
        ...
        for path_model_dir in args.path_model_dir:
            ...
            if not args.no_prediction:
                for idx_c in range(prediction.size()[0]):
                    ...
                    # Convert to uint8 (normalize to 0-255 range)
                    pred_min = pred_c.min()
                    pred_max = pred_c.max()
                    if pred_max > pred_min:
                        pred_c = ((pred_c - pred_min) / (pred_max - pred_min) * 255).astype(np.uint8)
                    else:
                        pred_c = np.zeros_like(pred_c, dtype=np.uint8)
