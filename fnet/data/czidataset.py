...
def _convert_to_uint8(im):
    """Convert image to uint8 by normalizing to [0, 255] range."""
    ...
    if im_max > im_min:
        ...
        im_normalized = im
    return (im_normalized * 255).astype(np.uint8)
    ...
    def __getitem__(self, index):
        ...
        if has_target and self.transform_target is not None:
            for t in self.transform_target:
                im_out[1] = t(im_out[1])
        im_out = [torch.from_numpy(_convert_to_uint8(im)) for im in im_out]
