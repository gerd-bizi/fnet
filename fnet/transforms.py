...
def norm_around_center(ar: np.ndarray, z_center: Optional[int] = None):
    ...
    if z_center is None:
        z_center = ar.shape[0] // 2
    z_start = max(0, min(z_center - 16, ar.shape[0] - 32))
    chunk = ar[z_start:z_start+32]
    mean = float(chunk.mean())
    std = float(chunk.std())
    eps = 1e-6
    std = std if std > 0 else eps
    norm_ar = (ar - mean) / std
    norm_ar = np.clip(norm_ar, -3, 3)  # restrict outliers
    norm_ar = ((norm_ar + 3) / 6) * 255  # scale to [0, 255]
    return norm_ar.astype(np.uint8)